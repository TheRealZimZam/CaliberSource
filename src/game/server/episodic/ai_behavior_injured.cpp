...
#include "ai_behavior_injured.h"

ConVar g_debug_injured_follow( "g_debug_injured_follow", "0" );
ConVar injured_help_plee_range( "injured_help_plee_range", "256" );		//Distance that is considered too far away from target
...
{
	// This seems like this should be a part of the base-class
...
//-----------------------------------------------------------------------------
// Purpose: Determines if we should run away from enemies or stay put
	...
	if ( AI_IsSinglePlayer() && bInDanger )
	{
		...
		if ( bWarnPlayer )
		{
			...
			if ( HasCondition( COND_SEE_PLAYER ) == false )	//|| flPlayerDistSqr > Square( 128 )
			{
				...
				{
					pPlayer->EmitSound( "npc_alyx.injured_too_far" );	//DISGOSTANG
			...
			else
			{
				...
				m_flNextWarnTime = gpGlobals->curtime + random->RandomFloat( 4.0f, 6.0f );
			}
