...
// STORY CHARACTER
// Purpose: A walking, pissing vending machine of bad one-liners and swears that would
// 			make a sailor blush, complete with a Big Apple accent.
//
//=============================================================================//
// TODO:
...
// Define stuff
#define BOSTON_MODEL "models/Barney.mdl"

#define BOSTON_MIN_MOB_DIST_SQR Square(120)		// Any enemy closer than this adds to the 'mob' 
#define BOSTON_MIN_CONSIDER_DIST Square(1200)	// Only enemies within this range are counted and considered to generate AI speech

#define BOSTON_MIN_ENEMY_DIST_TO_CROUCH			360			// Minimum distance that enemy must be to crouch
#define BOSTON_MIN_ENEMY_HEALTH_TO_CROUCH			20 		// Dont crouch for small enemies like headcrabs and the like
#define BOSTON_CROUCH_DELAY						5			// Time after crouching before Boston will consider crouching again
...
void CNPC_Boston::PainSound( const CTakeDamageInfo &info )
{
...
void CNPC_Boston::BarnacleDeathSound( void )
{
...
PassengerState_e CNPC_Boston::GetPassengerState( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Boston::PrescheduleThink( void )
{
	...
	if ( m_bIsFlashlightBlind )
	{
		if ( m_fStayBlindUntil < gpGlobals->curtime )
		{
				...
				!BlindedByFlare() )
			{
				...
				ADD_DEBUG_HISTORY( HISTORY_BOSTON_BLIND, UTIL_VarArgs( "(%0.2f) Boston: end blind scene '%s'\n", gpGlobals->curtime, STRING(m_iszCurrentBlindScene) ) );
...
//-----------------------------------------------------------------------------
void CNPC_Boston::PlayerHasIlluminatedNPC( CBasePlayer *pPlayer, float flDot )
{
 	if ( m_bIsFlashlightBlind )
		return;
	...
	if ( PlayerFlashlightOnMyEyes( pPlayer ) )
	{
		...
		if ( SpeakIfAllowed( "TLK_FLASHLIGHT_ILLUM", NULL, false, szResponse, AI_Response::MAX_RESPONSE_NAME  ) )
		{
			...
			ADD_DEBUG_HISTORY( HISTORY_BOSTON_BLIND, UTIL_VarArgs( "(%0.2f) Boston: start flashlight blind scene '%s'\n", gpGlobals->curtime, STRING(m_iszCurrentBlindScene) ) );
...
//-----------------------------------------------------------------------------
bool CNPC_Boston::CanBeBlindedByFlashlight( bool bCheckLightSources )
{
	...
	*/
	...
	// can't be blinded if Boston is near a light source
...
//-----------------------------------------------------------------------------
bool CNPC_Boston::PlayerFlashlightOnMyEyes( CBasePlayer *pPlayer )
{
...
//-----------------------------------------------------------------------------
void CNPC_Boston::CheckBlindedByFlare( void )
{
	if ( m_bIsFlashlightBlind )
		return;
	...
	if ( BlindedByFlare() )
	{
		...
		{
			m_iszCurrentBlindScene = AllocPooledString( szResponse );
			ADD_DEBUG_HISTORY( HISTORY_BOSTON_BLIND, UTIL_VarArgs( "(%0.2f) Boston: start flare blind scene '%s'\n", gpGlobals->curtime, 
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Boston::BlindedByFlare( void )
{
...
//-----------------------------------------------------------------------------
void CNPC_Boston::EnemyIgnited( CAI_BaseNPC *pVictim )
{
	if ( FVisible( pVictim ) )
	{
...
//-----------------------------------------------------------------------------
// Purpose: Custom AI for Boston while in combat
//-----------------------------------------------------------------------------
void CNPC_Boston::DoCustomCombatAI( void )
{
...
bool CNPC_Boston::RunningPassengerBehavior( void )
{
...
//-----------------------------------------------------------------------------
// Purpose: Handle "mobbed" combat condition when Boston is overwhelmed by force
// Copied from Alyx
//-----------------------------------------------------------------------------
void CNPC_Boston::DoMobbedCombatAI( void )
{
	...
	{
		if ( IRelationType( pEMemory->hEnemy ) != D_NU && IRelationType( pEMemory->hEnemy ) != D_LI && pEMemory->hEnemy->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <= BOSTON_MIN_CONSIDER_DIST )
		{
			...
			{
				if( pEMemory->hEnemy->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <= BOSTON_MIN_MOB_DIST_SQR )
				{
	...
	// Boston conviently doesnt need to reload when mobbed in cqc.
...
//-----------------------------------------------------------------------------
bool CNPC_Boston::OnBeginMoveAndShoot()
{
...
void CNPC_Boston::SpeakAttacking( void )
{
