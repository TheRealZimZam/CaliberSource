...
//!!TODO;
// This gonna be a hell of a port... -M.M
...
class CFuncVehicle : public CBasePlatTrain
{
	DECLARE_CLASS( CFuncVehicle, CBasePlatTrain );
	DECLARE_SERVERCLASS();
...
void CFuncVehicle::KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(szKeyName, "height"))
	{
		m_height = atof(pkvd->szValue);
	}
	else if (FStrEq(szKeyName, "startspeed"))
	{
...
void CFuncVehicle::UpdateSound( void )
{
	...
	if (!m_soundPlaying)
	{
		...
		if (m_sounds < 5)
			EMIT_SOUND_DYN(ENT(pev), CHAN_ITEM, "plats/vehicle_brake1.wav", m_volume, ATTN_NORM, 0, 100);
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char*)STRING(pev->noise), m_volume, ATTN_NORM, 0, (int) flpitch);
		m_soundPlaying = 1;
	} 
	else
	{
/*
		// update pitch
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, (char*)STRING(pev->noise), m_volume, ATTN_NORM, SND_CHANGE_PITCH, (int) flpitch);
*/
		...
		unsigned short us_volume = ( ( unsigned short )( m_volume * 40.0 ) & 0x003f );
