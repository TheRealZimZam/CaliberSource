...
void CBaseHudChatInputLine::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	// FIXME:  Outline
	vgui::HFont hFont = pScheme->GetFont( "Default" );	//Trebuchet18
	m_pPrompt->SetFont( hFont );
	m_pInput->SetFont( hFont );

	m_pInput->SetFgColor( pScheme->GetColor( "Chat.TypingText", Color( 255, 255, 255, 255 )) );
...
void CHudChatFilterPanel::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	SetFgColor( pScheme->GetColor("Chat.Panel", GetFgColor()) );
}
...
void CHudChatHistory::ApplySchemeSettings( vgui::IScheme *pScheme )
{
	...
	SetFont( pScheme->GetFont( "Marlett" ) );
	SetAlpha( 255 );
}
...
void CBaseHudChat::MsgFunc_SayText2( bf_read &msg )
{
	...
	{
		// print raw chat text
		Printf( GetFilterForString(untranslated_msg_text), "%s", ansiString );
	}
}
...
void CBaseHudChat::MsgFunc_VoiceSubtitle( bf_read &msg )
{
	...
	Printf( CHAT_FILTER_NONE, "%c(%s) %s%c: %s", COLOR_PLAYERNAME, szPrefix, GetDisplayedSubtitlePlayerName( client ), COLOR_NORMAL, ConvertCRtoNL( szString ) );
...
void CBaseHudChat::OnTick( void )
{
	...
	CBaseHudChatLine *line = m_ChatLines;
...
void CBaseHudChat::Printf( int iFilter, const char *fmt, ... )
{
	...
	if ( pos )
	{
		...
		// Print first part
		Printf( iFilter, msgcopy );

		// Print remainder
		Printf( iFilter, &msgcopy[ offset ] + 1 );
		return;
	}
...
void CBaseHudChat::FireGameEvent( IGameEvent *event )
{
	...
	if ( Q_strcmp( "hltv_chat", eventname ) == 0 )
	{
		...
		Printf( CHAT_FILTER_NONE, "(SourceTV) %s", event->GetString( "text" ) );
	}
#endif
}