...
class CNPC_Barney_New : public CAI_BaseActor
{
public:
	DECLARE_CLASS( CNPC_Barney_New, CAI_BaseActor );
	DECLARE_SERVERCLASS();
	DECLARE_DATADESC();
...
LINK_ENTITY_TO_CLASS( npc_barney_new, CNPC_Barney_New );

float	CNPC_Barney_New::m_flTimeFlipOff = 0;
float	CNPC_Barney_New::m_flTimeConspire = 0;
bool	CNPC_Barney_New::m_fCanSleep = true;


//---------------------------------------------------------
// 
//---------------------------------------------------------
IMPLEMENT_SERVERCLASS_ST(CNPC_Barney_New, DT_NPC_Barney)
END_SEND_TABLE()
...
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CNPC_Barney_New )

#if 0
	DEFINE_FIELD( CNPC_Barney_New, m_fIsRebelling, FIELD_BOOLEAN ),
	DEFINE_FIELD( CNPC_Barney_New, m_flBreatheTime, FIELD_FLOAT ),
#endif

	DEFINE_FIELD( CNPC_Barney_New, m_flStandStillTime, FIELD_TIME ),
	// DEFINE_FIELD( CNPC_Barney_New, m_cVisiblePolice, FIELD_INTEGER ) //recalc'd each think
	// DEFINE_FIELD( CNPC_Barney_New, m_cVisibleCitizens, FIELD_INTEGER ) //recalc'd each think

//	DEFINE_FIELD( CNPC_Barney_New, m_LeadBehavior, FIELD_EMBEDDED ),
//	DEFINE_FIELD( CNPC_Barney_New, m_FollowBehavior, FIELD_EMBEDDED ),
//	DEFINE_FIELD( CNPC_Barney_New, m_StandoffBehavior, FIELD_EMBEDDED ),
//	DEFINE_FIELD( CNPC_Barney_New, m_AssaultBehavior, FIELD_EMBEDDED ),

	// Function Pointers
	DEFINE_USEFUNC( CNPC_Barney_New, FollowUse ),
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Barney_New::Precache( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Barney_New::Spawn( void )
{
	Precache();
...
bool CNPC_Barney_New::CreateBehaviors()
{
...
// Output : 
//-----------------------------------------------------------------------------
Class_T	CNPC_Barney_New::Classify( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_Barney_New::GetSoundInterests ( void )
{
	return	SOUND_WORLD		|
			SOUND_COMBAT	|
			SOUND_PLAYER;
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Barney_New::IsWeaponPistol( void )
{
	if( GetActiveWeapon() )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Barney_New::HandleAnimEvent( animevent_t *pEvent )
{
	switch( pEvent->event )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Barney_New::OnBehaviorChangeStatus( CAI_BehaviorBase *pBehavior, bool fCanFinishSchedule )
{
...
int CNPC_Barney_New::SelectSchedule( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_Barney_New::TranslateSchedule( int scheduleType ) 
{
	switch( scheduleType )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Barney_New::PrescheduleThink( void )
{
...
// Output :
//-----------------------------------------------------------------------------
Activity CNPC_Barney_New::NPC_TranslateActivity( Activity activity )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Barney_New::RunAI( void )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Barney_New::OnSeeEntity( CBaseEntity *pEntity )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Barney_New::StartTask( const Task_t *pTask )
{
	switch( pTask->iTask )
	{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Barney_New::RunTask( const Task_t *pTask )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Barney_New::FValidateHintType( CAI_Hint *pHint )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Barney_New::FollowUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( m_pSquad && m_pSquad->NumMembers() > 1 )
	{
		...
		{
			CNPC_Barney_New *pBarney = dynamic_cast<CNPC_Barney_New *>(pSquadMember);
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Barney_New::Touch( CBaseEntity *pOther )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Barney_New::FInAimCone( const Vector &vecSpot )
{
...
AI_BEGIN_CUSTOM_NPC( npc_barney, CNPC_Barney_New )
