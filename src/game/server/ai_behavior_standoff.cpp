...
int CAI_StandoffBehavior::SelectScheduleEstablishAim( void )
{
	if ( HasCondition( COND_ENEMY_OCCLUDED ) )
	{
		...
		{
			// force a stand up, just in case. Also give a shout if you can.
...
void CAI_StandoffBehavior::UpdateBattleLines()
{
	if ( m_UpdateBattleLinesSemaphore.EnterThink() )
	{
		...
		// Hey Tom, maybe... we could do it at every-other think??
...
// NOTENOTE; add the activity for new weapons that might support cover animations here (tripod-machineguns, bazookas and such)
void CAI_MappedActivityBehavior_Temporary::UpdateTranslateActivityMap()
{
	...
	/*
	// Old mappings for old animations/postures
