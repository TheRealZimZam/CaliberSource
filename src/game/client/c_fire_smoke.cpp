...
#define	PARTICLE_FIRE 0		//Small flame sprites ontop of the main sprite
#define	EMBER_FIRE 1		//Costly but good-looking embers from smokey fires
...
void C_FireSmoke::AddFlames( void )
{
	...
	//FIXME; Particle fire is borked, it would be nice to have it working even if its never used
#if PARTICLE_FIRE
...
void C_FireSmoke::SpawnSmoke( void )
{
	...
	{
		sParticle->m_flLifetime		= 0.0f;
		sParticle->m_flDieTime		= random->RandomFloat( EMBER_LIFETIME * 0.75, EMBER_LIFETIME );
	...
	if ( pParticle )
	{
		...
		pParticle->m_uchColor[3] = random->RandomInt( 96, 224 );	// 128, 256

		pParticle->m_uchStartSize	= random->RandomFloat( 16.0f, 24.0f );	//24, 32
...
#define	ENTITY_FLAME_GLOW 1		//Do hitbox fires glow/smoke??
...
void C_EntityFlame::Simulate( void )
{
	...
	if ( IsEffectActive(EF_BRIGHTLIGHT) || IsEffectActive(EF_DIMLIGHT) )
	{
		...
		dl->color.g = 174;
		dl->color.b = 10;
		dl->radius = random->RandomFloat(384,416);
