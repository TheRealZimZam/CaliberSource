...
#define	HOUND_CIRCLE_RADIUS		500		//How far for the pack to circle around the target
#define	HOUND_MIN_ATTACK_RADIUS		100
...
// Interactions
//=========================================================
int	g_interactionHoundGroupAttack					= 0;
int	g_interactionHoundGroupRetreat					= 0;
int	g_interactionHoundGroupRalley					= 0;
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Hound::InitCustomSchedules(void) 
{
	INIT_CUSTOM_AI(CNPC_Hound);

	ADD_CUSTOM_TASK(CNPC_Hound,	TASK_HOUND_THREAT_DISPLAY);	//Stand and bark at the target
	ADD_CUSTOM_TASK(CNPC_Hound,	TASK_HOUND_FALL_ASLEEP);
	ADD_CUSTOM_TASK(CNPC_Hound,	TASK_HOUND_WAKE_UP);
	ADD_CUSTOM_TASK(CNPC_Hound,	TASK_HOUND_HOP_BACK);	//Hop back in suprise from big damage/threat

	ADD_CUSTOM_TASK(CNPC_Hound,	TASK_HOUND_GET_PATH_TO_CIRCLE);
	ADD_CUSTOM_TASK(CNPC_Hound,	TASK_HOUND_REVERSE_STRAFE_DIR);
	
	ADD_CUSTOM_CONDITION(CNPC_Hound,	COND_HOUND_GROUP_ATTACK);
	ADD_CUSTOM_CONDITION(CNPC_Hound,	COND_HOUND_GROUP_RETREAT);

	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_HOP_RETREAT);
	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_RANGE_ATTACK1);
	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_ATTACK_STRAFE);
	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_ATTACK_STRAFE_REVERSE);
	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_GROUP_ATTACK);
	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_GROUP_RETREAT);
	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_CHASE_ENEMY);
	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_COVER_WAIT);
	ADD_CUSTOM_SCHEDULE(CNPC_Hound,	SCHED_HOUND_GROUP_RALLEY);
	
	ADD_CUSTOM_ACTIVITY(CNPC_Hound,	ACT_HOUND_GUARD);

	g_interactionHoundGroupAttack				= CBaseCombatCharacter::GetInteractionID();
	g_interactionHoundGroupRetreat				= CBaseCombatCharacter::GetInteractionID();
	g_interactionHoundGroupRalley				= CBaseCombatCharacter::GetInteractionID();

	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_HOP_RETREAT);
	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_RANGE_ATTACK1);
	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_ATTACK_STRAFE);
	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_ATTACK_STRAFE_REVERSE);
	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_GROUP_ATTACK);
	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_GROUP_RETREAT);
	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_CHASE_ENEMY);
	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_COVER_WAIT);
	AI_LOAD_SCHEDULE(CNPC_Hound,	SCHED_HOUND_GROUP_RALLEY);
}

LINK_ENTITY_TO_CLASS( npc_houndeye, CNPC_Hound );
IMPLEMENT_CUSTOM_AI( npc_houndeye, CNPC_Hound );
#ifndef HL1_DLL
LINK_ENTITY_TO_CLASS( monster_houndeye, CNPC_Hound );
#endif

BEGIN_DATADESC( CNPC_Hound )
...
// relationship table.
//=========================================================
Class_T	CNPC_Hound::Classify ( void )
{
	return	CLASS_HOUND;
}
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Hound::RangeAttack1Conditions ( float flDot, float flDist )
{
	...
	if (tr.startsolid)
	{
		...
		if (pEntity->Classify() == CLASS_HOUND)
		{
			return( COND_NONE );
		}
	}
	...
	if (flDist > ( HOUND_CIRCLE_RADIUS * 0.5 ))
	{
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Hound::GetSoundInterests( void )
{
	return	SOUND_WORLD		|
			SOUND_COMBAT	|
			SOUND_PLAYER	|
...
//=========================================================
float CNPC_Hound::MaxYawSpeed ( void )
{
	int ys = 90;
...
//=========================================================
void CNPC_Hound::HandleAnimEvent( animevent_t *pEvent )
{
	switch ( pEvent->event )
	{
		case HOUND_AE_WARN:
		...
		case HOUND_AE_CLOSE_EYE:
			if ( !m_fDontBlink )
			{
			//<<TEMP>>	pev->skin = HOUND_EYE_FRAMES - 1;
			}
			break;
...
void CNPC_Hound::Spawn()
{
	...
	SetCollisionGroup( HL2COLLISION_GROUP_HOUND ); 
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Hound::SpeakSentence( int sentenceType )
{
...
// IdleSound
//=========================================================
void CNPC_Hound::IdleSound ( void )
{
	if (!FOkToMakeSound())
	{
		return;
...
// IdleSound
//=========================================================
void CNPC_Hound::WarmUpSound ( void )
{
...
// WarnSound 
//=========================================================
void CNPC_Hound::WarnSound ( void )
{
...
// AlertSound 
//=========================================================
void CNPC_Hound::AlertSound ( void )
{
...
// DeathSound 
//=========================================================
void CNPC_Hound::DeathSound( const CTakeDamageInfo &info )
{
...
// PainSound 
//=========================================================
void CNPC_Hound::PainSound( const CTakeDamageInfo &info )
{
...
//=========================================================
void CNPC_Hound::WriteBeamColor ( void )
{
...
/*
	if (GetCollisionGroup() == HL2COLLISION_GROUP_HOUND)
	{
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_Hound::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	if (m_pSquad && random->RandomInt(0,10) == 10)
	{
		...
		m_pSquad->BroadcastInteraction( g_interactionHoundGroupRetreat, NULL, this );
	}
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Hound::Event_Killed( const CTakeDamageInfo &info )
{
	EmitSound( "NPC_Houndeye.Retreat" );
	m_flSoundWaitTime = gpGlobals->curtime + 1.0;
...
// SonicAttack
//=========================================================
void CNPC_Hound::SonicAttack ( void )
{
	...
	for ( CEntitySphereQuery sphere( GetAbsOrigin(), HOUND_CIRCLE_RADIUS ); pEntity = sphere.GetCurrentEntity(); sphere.NextEntity() )
	{
		if (pEntity->Classify()	== CLASS_HOUND)
		{
			continue;
		...
		if ( pEntity->m_takedamage != DAMAGE_NO || pPhysicsObject)
		{
			...
			float flDamageAdjuster = 1-( flDist / HOUND_CIRCLE_RADIUS );
...
// start task
//=========================================================
void CNPC_Hound::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	...
	{
		float			flMaxRange	= HOUND_CIRCLE_RADIUS * 0.9;
		float			flMinRange	= HOUND_MIN_ATTACK_RADIUS;
		Vector 			posLos;
		bool			foundLos	= false;
...
// RunTask 
//=========================================================
void CNPC_Hound::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_HOUND_THREAT_DISPLAY:
	...
	case TASK_HOUND_CLOSE_EYE:
		{
			/*//<<TEMP>>
			if ( pev->skin < HOUND_EYE_FRAMES - 1 )
			{
				pev->skin++;
			}
			*/
...
// Output  :
//------------------------------------------------------------------------------
void CNPC_Hound::PrescheduleThink ( void )
{
...
// Output :
//-----------------------------------------------------------------------------
Activity CNPC_Hound::NPC_TranslateActivity( Activity eNewActivity )
{
...
// Output  :
//------------------------------------------------------------------------------
int CNPC_Hound::TranslateSchedule( int scheduleType )
{
	switch( scheduleType )
	{
...
// Output  :
//------------------------------------------------------------------------------
bool CNPC_Hound::IsAnyoneInSquadAttacking( void )
{
	if (!m_pSquad)
	{
		return false;
...
// SelectSchedule
//=========================================================
int CNPC_Hound::SelectSchedule( void )
{
	switch	( m_NPCState )
	{
	case NPC_STATE_IDLE:
			...
			if ( HasCondition( COND_CAN_RANGE_ATTACK1 ) )
			{
				if (m_pSquad && random->RandomInt(0,4) == 0)
				{
					if (!IsAnyoneInSquadAttacking())
					{
						...
						m_pSquad->BroadcastInteraction( g_interactionHoundGroupAttack, NULL, this );
						return SCHED_HOUND_GROUP_ATTACK;
					}
				}
				//<<TEMP>>comment
			...
			else
			{
				if (m_pSquad && random->RandomInt(0,5) == 0)
				{
					if (!IsAnyoneInSquadAttacking())
					{
						...
						m_pSquad->BroadcastInteraction( g_interactionHoundGroupRalley, NULL, this );
...
//-----------------------------------------------------------------------------
bool CNPC_Hound::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	if (interactionType == g_interactionHoundGroupAttack)
	{
		SetCondition(COND_HOUND_GROUP_ATTACK);
		return true;
	}
	else if (interactionType == g_interactionHoundGroupRetreat)
	{
		SetCondition(COND_HOUND_GROUP_RETREAT);
		return true;
	}
	else if (interactionType == g_interactionHoundGroupRalley)
	{
