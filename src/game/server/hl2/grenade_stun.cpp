...
// How long the player stuns enemies
ConVar sk_plr_stungrenade_duration	( "sk_plr_stungrenade_duration","5");
// How long the player gets stunned
ConVar sk_npc_stungrenade_duration	( "sk_npc_stungrenade_duration","3");
// How long the timer is for the player
ConVar sk_plr_stungrenade_timer	( "sk_plr_stungrenade_duration","3");
// How long the timer is for npcs
ConVar sk_npc_stungrenade_timer	( "sk_npc_stungrenade_duration","4");
// Radius of the stun
ConVar sk_stungrenade_radius	( "sk_stungrenade_radius", "256");

#define STUNGRENADE_MODEL "models/Weapons/w_grenade.mdl"

class CGrenadeStun : public CBaseGrenade
{
	DECLARE_CLASS( CGrenadeStun, CBaseGrenade );
...
LINK_ENTITY_TO_CLASS( npc_grenade_stun, CGrenadeStun );

BEGIN_DATADESC( CGrenadeStun )
...
// Purpose: 
//-----------------------------------------------------------------------------
CGrenadeStun::~CGrenadeStun( void )
{
}

void CGrenadeStun::Spawn( void )
{
	Precache( );
	...
	{
		m_flDamage		= sk_plr_stungrenade_duration.GetFloat();
		m_DmgRadius		= sk_stungrenade_radius.GetFloat();
	}
	else
	{
		m_flDamage		= sk_npc_stungrenade_duration.GetFloat();
		m_DmgRadius		= sk_stungrenade_radius.GetFloat();
	}
	...
	// First and only BLIP
	BlipSound();
//	m_flNextBlipTime = gpGlobals->curtime + STUN_GRENADE_BLIP_FREQUENCY;
...
void CGrenadeStun::Precache( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeStun::OnRestore( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeStun::CreateEffects( void )
{
	// Start up the eye glow
	// NOTENOTE; CHANGE THIS TO BLUE
...
bool CGrenadeStun::CreateVPhysics()
{
...
void CGrenadeStun::VPhysicsUpdate( IPhysicsObject *pPhysics )
{
...
void CGrenadeStun::SetTimer( float detonateDelay, float warnDelay )
{
	m_flDetonateTime = gpGlobals->curtime + detonateDelay;
	m_flWarnAITime = gpGlobals->curtime + warnDelay;
	SetThink( &CGrenadeStun::DelayThink );
	SetNextThink( gpGlobals->curtime );

	CreateEffects();
}

void CGrenadeStun::OnPhysGunPickup( CBasePlayer *pPhysGunUser, PhysGunPickup_t reason )
{
	SetThrower( pPhysGunUser );

#ifdef HL2MP
	SetTimer( STUN_GRENADE_GRACE_TIME_AFTER_PICKUP, STUN_GRENADE_GRACE_TIME_AFTER_PICKUP / 2);

//	BlipSound();
//	m_flNextBlipTime = gpGlobals->curtime + STUN_GRENADE_BLIP_FAST_FREQUENCY;
	m_bHasWarnedAI = true;
#else
	if( IsX360() )
	{
...
void CGrenadeStun::SetVelocity( const Vector &velocity, const AngularImpulse &angVelocity )
{
...
int CGrenadeStun::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
...
bool CGrenadeStun::HandleInteraction(int interactionType, void *data, CBaseCombatCharacter* sourceEnt)
{
	// allow stunnades to be grabbed by barnacles. 
	if ( interactionType == g_interactionBarnacleVictimGrab )
	{
		...
		SetTimer( timer, timer - STUN_GRENADE_WARN_TIME );
...
void CGrenadeStun::InputSetTimer( inputdata_t &inputdata )
{
	SetTimer( inputdata.value.Float(), inputdata.value.Float() - STUN_GRENADE_WARN_TIME );
}
...
CBaseGrenade *Stungrenade_Create( const Vector &position, const QAngle &angles, const Vector &velocity, const AngularImpulse &angVelocity, CBaseEntity *pOwner, float timer, bool combineSpawned )
{
	...
	CGrenadeStun *pGrenade = (CGrenadeStun *)CBaseEntity::Create( "npc_grenade_stun", position, angles, pOwner );
	
	pGrenade->SetTimer( timer, timer - STUN_GRENADE_WARN_TIME );
...
{
	const CGrenadeStun *pFrag = dynamic_cast<const CGrenadeStun *>( pEntity );
	if ( pStun )
	{
		return pStun->WasPunted();
	}
...
void CGrenadeStun::Explode( trace_t *pTrace, int bitsDamageType )
{
#if !defined( CLIENT_DLL )
	...
	EmitSound( "StunGrenade.Explode" );

	// Concussion.
	color32 Concussion = { 255, 255, 255, 64 };
...
//	else
//	{
//		UTIL_ScreenFade( pPlayer, Concussion, 0.2f, 0, (FFADE_OUT|FFADE_PURGE|FFADE_STAYOUT) );	
//	}
	// !!!Example
	...
	if ( pPlayer )
	{
		gamestats->Event_WeaponHit( pPlayer, true, "weapon_stun", info );
	}
#endif
...
void CGrenadeStun::Detonate( void )
{
