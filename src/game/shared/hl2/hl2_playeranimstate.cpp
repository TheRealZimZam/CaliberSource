...
#define HL2MP_RUN_SPEED				170.0f
#define HL2MP_WALK_SPEED			100.0f
...
CHL2PlayerAnimState::CHL2PlayerAnimState()
{
	...
	// Don't initialize HL2MP specific variables here. Init them in InitHL2AnimState()
}
...
// Input  : *pPlayer - 
//-----------------------------------------------------------------------------
void CHL2PlayerAnimState::Init( CHL2_Player *pPlayer )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHL2PlayerAnimState::Update( float eyeYaw, float eyePitch )
{
	// Profile the animation update.
	VPROF( "CHL2PlayerAnimState::Update" );

	// Get the HL2MP player.
	CHL2_Player *pHL2MPPlayer = GetHL2Player();
	if ( !pHL2MPPlayer )
		return;
	...
	{
		m_pHL2Player->SetPlaybackRate( 1.0f );
	}
#endif
}
...
void CHL2PlayerAnimState::DoAnimationEvent( PlayerAnimEvent_t event, int nData )
{
	...
	case PLAYERANIMEVENT_ATTACK_PRE:
		{
			if ( m_pHL2Player->GetFlags() & FL_DUCKING ) 
			{
...
// Input  : *idealActivity - 
//-----------------------------------------------------------------------------
bool CHL2PlayerAnimState::HandleSwimming( Activity &idealActivity )
{
...
bool CHL2PlayerAnimState::HandleJumping( Activity &idealActivity )
{
	...
	if ( m_bJumping )
	{
		...
		if ( m_pHL2Player->GetWaterLevel() >= WL_Waist )
		{
			m_bJumping = false;
			RestartMainSequence();
		}
		...
		{
			if ( m_pHL2Player->GetFlags() & FL_ONGROUND )
			{
				m_bJumping = false;
				RestartMainSequence();
				...
				if ( bNewJump )
				{
					RestartGesture( GESTURE_SLOT_JUMP, ACT_LAND );					
				}
			}
		}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHL2PlayerAnimState::EstimateYaw( void )
{
	// Get the frame time.
...
// Output : float
//-----------------------------------------------------------------------------
float CHL2PlayerAnimState::GetCurrentMaxGroundSpeed()
{
