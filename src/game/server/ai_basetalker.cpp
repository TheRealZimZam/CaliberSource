...
ConceptInfo_t g_ConceptInfos[] =
{
	...
	{ 	TLK_NOSHOOT,		SPEECH_PRIORITY,	-1,		-1,		-1,		-1,		-1,		-1,		AICF_DEFAULT,	},
	...
	{ 	TLK_DARKNESS_FOUNDPLAYER, SPEECH_IDLE,-1,	-1,		-1,		-1,		 -1,	-1,		AICF_DEFAULT,	},
...
void CAI_AllySpeechManager::OnSpokeConcept( CAI_BaseTalker *pPlayerAlly, AIConcept_t concept, AI_Response *response )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::IsPlayerAlly( CBasePlayer *pPlayer ) 
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::InputMakeGameEndAlly( inputdata_t &inputdata )
{
	m_bGameEndAlly = true;
}

void CAI_BaseTalker::InputMakeRegularAlly( inputdata_t &inputdata )
{
	m_bGameEndAlly = false;
}

void CAI_BaseTalker::DisplayDeathMessage( void )
{
...
//-----------------------------------------------------------------------------
// NPCs derived from CAI_BaseTalker should call this in precache()
//-----------------------------------------------------------------------------
void CAI_BaseTalker::TalkInit( void )
{
}	

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::GatherConditions( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::GatherEnemyConditions( CBaseEntity *pEnemy )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::PrescheduleThink( void )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CAI_BaseTalker::SelectSchedule( void )
{
	// why is this in select sched?? shouldnt this be in takedamage?
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::SelectSpeechResponse( AIConcept_t concept, const char *pszModifiers, CBaseEntity *pTarget, AISpeechSelection_t *pSelection )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::SetPendingSpeech( AIConcept_t concept, AI_Response *pResponse )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::ClearPendingSpeech()
{
...
bool CAI_BaseTalker::SelectIdleSpeech( AISpeechSelection_t *pSelection )
{
...
bool CAI_BaseTalker::SelectInterjection()
{
	if ( HasPendingSpeech() )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::SelectPlayerUseSpeech()
{
...
bool CAI_BaseTalker::SelectQuestionAndAnswerSpeech( AISpeechSelection_t *pSelection )
{
	...
	CAI_BaseTalker *pFriend = dynamic_cast<CAI_BaseTalker *>(FindSpeechTarget( AIST_NPCS ));
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAI_BaseTalker::PostSpeakDispatchResponse( AIConcept_t concept, AI_Response *response )
{
...
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::SelectQuestionFriend( CBaseEntity *pFriend, AISpeechSelection_t *pSelection )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CAI_BaseTalker::InputAnswerQuestion( inputdata_t &inputdata )
{
	AnswerQuestion( dynamic_cast<CAI_BaseTalker *>(inputdata.pActivator), inputdata.value.Int(), false );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CAI_BaseTalker::InputAnswerQuestionHello( inputdata_t &inputdata )
{
	AnswerQuestion( dynamic_cast<CAI_BaseTalker *>(inputdata.pActivator), inputdata.value.Int(), true );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CAI_BaseTalker::AnswerQuestion( CAI_BaseTalker *pQuestioner, int iQARandomNum, bool bAnsweringHello )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CAI_BaseTalker::SelectNonCombatSpeech( AISpeechSelection_t *pSelection )
{
	bool bResult = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CAI_BaseTalker::SelectNonCombatSpeechSchedule()
{
	if ( !HasPendingSpeech() )
	{
		AISpeechSelection_t selection;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CAI_BaseTalker::TranslateSchedule( int schedule )
{
	if ( ( GetState() == NPC_STATE_IDLE || GetState() == NPC_STATE_ALERT ) && 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::StartTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::RunTask( const Task_t *pTask )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::TaskFail( AI_TaskFailureCode_t code )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::ClearTransientConditions()
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::Touch( CBaseEntity *pOther )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::OnKilledNPC( CBaseCombatCharacter *pKilled )
{
	if ( pKilled )
	{
		if ( !pKilled->IsNPC() || 
...
//-----------------------------------------------------------------------------
Disposition_t CAI_BaseTalker::IRelationType( CBaseEntity *pTarget )
{
#ifdef ALLIES_CAN_BE_PROVOKED
	if ( pTarget->IsPlayer() )
	{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CAI_BaseTalker::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CAI_BaseTalker::TakeHealth( float flHealth, int bitsDamageType )
{
	int intPortion;
	float floatPortion;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::Event_Killed( const CTakeDamageInfo &info )
{
	// notify the player
	if ( IsInPlayerSquad() )
	{
...
bool CAI_BaseTalker::CreateVPhysics()
{
...
//-----------------------------------------------------------------------------
void CAI_BaseTalker::DeathSound( const CTakeDamageInfo &info )
{
...
//-----------------------------------------------------------------------------
CBaseEntity *CAI_BaseTalker::EyeLookTarget( void )
{
...
//-----------------------------------------------------------------------------
CBaseEntity *CAI_BaseTalker::FindNamedEntity( const char *pszName, IEntityFindFilter *pFilter )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::IsValidSpeechTarget( int flags, CBaseEntity *pEntity )
{
	if ( pEntity == this )
		return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CAI_BaseTalker::FindSpeechTarget( int flags )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::CanPlaySentence( bool fDisregardState ) 
{ 
	if ( fDisregardState )
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CAI_BaseTalker::PlayScriptedSentence( const char *pszSentence, float delay, float volume, soundlevel_t soundlevel, bool bConcurrent, CBaseEntity *pListener )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CAI_BaseTalker::DeferAllIdleSpeech( float flDelay, CAI_BaseNPC *pIgnore )
{
	CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
	if ( flDelay == -1 )
	{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
bool CAI_BaseTalker::IsOkToSpeak( ConceptCategory_t category, bool fRespondingToPlayer )
{
	CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
bool CAI_BaseTalker::IsOkToSpeak( void )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
bool CAI_BaseTalker::IsOkToCombatSpeak( void )
{
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
bool CAI_BaseTalker::IsOkToSpeakInResponseToPlayer( void )
{
...
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::ShouldSpeakRandom( AIConcept_t concept, int iChance )
{
	CAI_AllySpeechManager *	pSpeechManager	= GetAllySpeechManager();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::IsAllowedToSpeak( AIConcept_t concept, bool bRespondingToPlayer ) 
{ 
	CAI_AllySpeechManager *	pSpeechManager	= GetAllySpeechManager();
	ConceptInfo_t *			pInfo			= pSpeechManager->GetConceptInfo( concept );
	ConceptCategory_t		category		= ( pInfo ) ? pInfo->category : SPEECH_IDLE;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::SpeakIfAllowed( AIConcept_t concept, const char *modifiers, bool bRespondingToPlayer, char *pszOutResponseChosen, size_t bufsize ) 
{ 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::OnSpokeConcept( AIConcept_t concept, AI_Response *response )
{
	CAI_AllySpeechManager *pSpeechManager = GetAllySpeechManager();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseTalker::OnStartSpeaking()
{
...
//-----------------------------------------------------------------------------
void CAI_BaseTalker::InputSpeakResponseConcept( inputdata_t &inputdata )
{
...
//-----------------------------------------------------------------------------
void CAI_BaseTalker::InputEnableSpeakWhileScripting( inputdata_t &inputdata )
{
	m_bCanSpeakWhileScripting = true;
}

void CAI_BaseTalker::InputDisableSpeakWhileScripting( inputdata_t &inputdata )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::SpeakMapmakerInterruptConcept( string_t iszConcept )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::CanRespondToEvent( const char *ResponseConcept )
{
	return true;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CAI_BaseTalker::RespondedTo( const char *ResponseConcept, bool bForce, bool bCancelScene )
{
	if ( bForce )
	{
...
AI_BEGIN_CUSTOM_NPC(talk_monster_base,CAI_BaseTalker)
