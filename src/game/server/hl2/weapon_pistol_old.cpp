...
#define	PISTOL_ACCURACY_SHOT_PENALTY_TIME		0.15f	// Applied amount of time each shot adds to the time we must recover from -- was 0.2, changed to match firerate
...
// Entire shoot code re-posted here because pistol does not inherit from cmachinegun
//-----------------------------------------------------------------------------
...
//	m_flNextPrimaryAttack
	while ( m_flNextSecondaryAttack <= gpGlobals->curtime )
	{
		...
		m_flNextSecondaryAttack = m_flNextSecondaryAttack + BurstFireRate;
		info.m_iShots++;
		if ( !BurstFireRate )
			break;
	}
