...
public:
	...
	int InternalDrawModel( int flags )
	{
		if ( IsFollowingEntity() && m_boneIndexAttached >= 0 )
		{
			...
			if ( follow )
			{
				//!!! TODO; Probably broken, just changed some things so it'd compile
				...
				ConcatTransforms( boneToWorld, localSpace, GetBoneForWrite(0) );	//3 - m_CachedBones[0]
				...
				Vector absOrigin;
				MatrixGetColumn( boneToWorld, 3, absOrigin );	// 1 - m_CachedBones[0]
